...
def get_pairwise_distances(images):
    ids = sorted(np.array(list(images.keys())))
    Rs = []
    ts = []
    for id_ in ids:
        image = images[id_]
...
def main(pose_path, output, num_matched, rotation_threshold=DEFAULT_ROT_THRESH):
    logger.info("Extracting image pairs from LiDAR poses.")
    ...
    # pairs = [ (str(i)+".png", str(j)+".png") for i, j in pairs]
    pairs = [ (ids[i], ids[j]) for i, j in pairs]

    logger.info(f"Found {len(pairs)} pairs.")
    with open(output, "w") as f:
        f.write("\n".join(" ".join(p) for p in pairs))

    print("Extracted pairs:", len(pairs))
